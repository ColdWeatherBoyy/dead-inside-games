...
router.post("/", async (req, res) => {
  try {
    ...
    if (!username || !password) {
      return res
        .status(404)
        .json("Please provide a unique username and password");
    }
...
router.post("/login", async (req, res) => {
  try {
    ...
    req.session.save(() => {
      ...
      res
        .status(200)
        .json({ message: "Logged in and ready to die (inside only)." });
    });
  } catch (err) {
    res.status(500).json(err);
  }
...
router.post("/logout", async (req, res) => {
  try {
    if (req.session.loggedIn) {
      ...
      res
        .status(200)
        .json("You are now logged out. I hope you feel alive again");
    } else {
